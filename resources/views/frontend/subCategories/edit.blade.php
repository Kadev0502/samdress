    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.sub-categories.update", [$subCategory->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control ckeditor" name="description" id="description">{!! old('description', $subCategory->description) !!}</textarea>
                        ...
                        <div class="form-group">
                            <label class="required" for="categories">{{ trans('cruds.subCategory.fields.category') }}</label>
                            ...
                            <select class="form-control select2" name="categories[]" id="categories" multiple required>
                                @foreach($categories as $id => $category)
                                    <option value="{{ $id }}" {{ (in_array($id, old('categories', [])) || $subCategory->categories->contains($id)) ? 'selected' : '' }}>{{ $category }}</option>
                                @endforeach
                            </select>
          ...
          return loader.file
            .then(function (file) {
              return new Promise(function(resolve, reject) {
                ...
                xhr.open('POST', '/admin/sub-categories/ckmedia', true);
                ...
                data.append('crud_id', '{{ $subCategory->id ?? 0 }}');
                xhr.send(data);
              });
            })
        }
