        ...
        <div class="col-md-12">
            @can('supplier_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.suppliers.create') }}">
                            ...
                            <tbody>
                                @foreach($suppliers as $key => $supplier)
                                    <tr data-entry-id="{{ $supplier->id }}">
                                        ...
                                        <td>
                                            {{ $supplier->currency->currency_code ?? '' }}
                                        </td>
                                        <td>
                                            @can('supplier_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.suppliers.show', $supplier->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('supplier_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.suppliers.edit', $supplier->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('supplier_delete')
                                                <form action="{{ route('frontend.suppliers.destroy', $supplier->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('supplier_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.suppliers.massDestroy') }}",
